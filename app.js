...
function snapPlayerToRoad(force = false){
  ...
  if(canPlayerStandAt(current)) return;
  const snapped = snapCoordToNearestRoad(current, force ? 200 : 92);
  if(!snapped) return;
  if(haversineMeters(current, snapped) > (force ? 0.8 : 0.45)){
...
function tryMoveWithCollision(mx, my){
  ...
  for(const [nx, ny] of candidates){
    ...
    const snappedCoord = snapCoordToNearestRoad(nextCoord, 84);
